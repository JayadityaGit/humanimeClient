...
const Streaming = () => {
  ...
  return (
    <div className='streaming p-5 grid grid-cols-1 gap-y-11 lg:grid-cols-2 items-start lg:p-10 lg:gap-x-4'>
             ...
             <div className='actualSubButtons flex space-x-2'>
                 {
                  sub.length !==0 ? sub.map((value)=>{
                    return(
                      <button  className={clickedSubServerButton==value.serverName?'border px-2 py-1 rounded-md text-sm bg-lime-300':'border px-2 py-1 rounded-md text-sm'} onClick={
                        async()=>{
                  ...
                  <p>click an episode and the server button to stop getting rickrolled.</p>
                 }
             </div>
              ...
              <div className='actualDubButtons flex space-x-2'>
                  {
                   dub.length !== 0 ?  dub.map((value)=>{
                      return(
                        <button className={clickedDubServerButton==value.serverName?'border px-2 py-1 rounded-md text-sm bg-lime-300':'border px-2 py-1 rounded-md text-sm'} onClick={
                          async()=>{
      ...
      {episodes.map((ep, index)=>{
          return(
            <button className={ep.episodeId == episodeId?'border py-1 bg-lime-300':'border-solid border-2 py-1 rounded-lg'} onClick={
              async()=>{
